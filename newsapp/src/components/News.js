...
"content": "In the summer of 2024, we saw no major superstars change teams. And rarely does an offseason such as that have as much of an impact on the following season's title race. Of the four teams that reache… [+16329 chars]"
  ...
  async componentDidMount(){
    console.log("cmd")
    let url=`https://newsapi.org/v2/top-headlines?country=us&category=business&apiKey=56114773742443238d68f1a90ba5175e&page=1&pagesize=${this.props.pagesize}`;
     let data=await fetch(url)
     let parsesdata=await data.json()
     console.log(parsesdata);
     this.setState({articles:parsesdata.articles,totalResults:parsesdata.totalResults})
     
  }
  handlepreviews=async()=>{
    console.log("cmd")
     let url=`https://newsapi.org/v2/top-headlines?country=us&category=business&apiKey=56114773742443238d68f1a90ba5175e&page=${this.state.page -1}&pagesize=${this.props.pagesize}`;
     let data=await fetch(url)
     let parsesdata=await data.json()
     console.log(parsesdata);
        ...
        <div className='row my-3'>
          ...
          {this.state.articles.map((element)=>{
              return  <div className='col-md-4' key={element.url}>
                 <NewsItems title={element.title} descrption={element.description} imgurl={element.urlToImage} newurl={element.url}/>
            </div>
          })}
